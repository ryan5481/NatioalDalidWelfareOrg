...
const SuperAdminDashboard = () => {
    ...
    function getTotalscholarshhips(studentsList) {
        return studentsList.reduce((sum, student) => {
            for (let i = 1; i <= 5; i++) {
                const cartageKey = `scholarship${i}Cartage`;
                const cartageValue = student[cartageKey].trim();
                sum += cartageValue && typeof cartageValue === 'string' && cartageValue.trim() !== '' ? 1 : 0;
            }
            return sum;
        }, 0);
    }
    ...
    const basicEducationGrades = ["PreK", "K1", "K2", "K3", "Grade1", "Grade2", "Grade3", "Grade4", "Grade5", "Grade6", "Grade7", "Grade8"];
