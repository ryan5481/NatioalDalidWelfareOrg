...
const StudentProfileForm = ({ setIsCreateNewUserActive, fetchData }) => {
  ...
  const submitForm = async (formData) => {
    try {
      ...
      // Append each field from your formData to formDataToSend
      Object.keys(formData).forEach((key) => {
        formDataToSend.append(key, formData[key]);
      });
  
      // Append the file separately
      formDataToSend.append('profileImageName', selectedImage);
  
      const res = await axios.post(`${baseUrl}/create-student-profile`, formDataToSend, {
        headers: {
