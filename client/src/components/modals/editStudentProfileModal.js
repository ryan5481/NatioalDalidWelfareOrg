...
import {
  useToast, Grid, Image, Box, FormLabel, Select, useDisclosure, Switch, Input,
  Button, Modal, ModalOverlay, ModalContent, ModalHeader, ModalCloseButton, ModalBody, ModalFooter, Text, HStack, FormControl, Radio,RadioGroup, Stack
...
const EditStudentProfileModal = ({ isOpen, onClose, data, scholarshipProject }) => {
  ...
  const [isCheck2FaDialogOpen, setIsCheck2FaDialogOpen] = useState(null);
  const dalitEthnicitiesList = ['Badi', 'Gandarva', 'Madeshi Origin', 'Pariyar', 'Sarki', 'Viswakarma']
  const [statusDisabled, setStatusDisabled] = useState(false); //disable or un-disable Alumni form
  ...
  const handleInputChange = (event) => {
    ...
    if (scholarshipProject !== "prlEth") {
      ...
      // const updatedValue = name === 'isAlumni' ? value === 'true' : value;
...
// console.log(statusDisabled)
  ...
  useEffect(() => {
    setFormData({
      ...
      alumniOccupation: data?.alumniOccupation,
      alumniOrganization: data?.alumniOrganization,
      alumniPosition: data?.alumniPosition,
    })
  }, [])
  ...
  const handleSubmit = async (event) => {
    ...
    formData.isAlumni && formDataToUpdate.append('isAlumni', formData.isAlumni);
    // formData.status && formDataToUpdate.append('status', formData.status);

    // formData.alumniOrganization && formDataToUpdate.append('alumniOrganization', formData.alumniOrganization);
    // formData.alumniOccupation && formDataToUpdate.append('alumniOrganization', formData.alumniOccupation);
    // formData.alumniPosition && formDataToUpdate.append('alumniPosition', formData.alumniPosition);
          ...
          <ModalContent >
            ...
            <ModalBody m={5} >
              ...
              >
                ...
                {/* ALUMNI */}
                <HStack mt={5} fontSize="20px" fontWeight="bold" align="start" >
                <Text  >Alumni: </Text>
                <Text>{data.isAlumni == true ? "Yes" : "No"}</Text>
              </HStack>
              {data.isAlumni == true &&
              <>
                ...
                <Grid gridTemplateColumns={"0.6fr 1fr 0.6fr 1fr 0.6fr 1fr"} gap={1}   >
                  <Text fontWeight="bold" >Organization:</Text>
                  <Text>{data.alumniOrganization}</Text>
                  <Text fontWeight="bold">Occupation:</Text>
                  <Text>{data.alumniOccupation}</Text>
                  <Text fontWeight="bold">Position:</Text>
                  <Text>{data.alumniPosition}</Text>
                </Grid>
                </>
                }
              </form>
